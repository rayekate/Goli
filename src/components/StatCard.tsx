  ...
  return (
    <div className="animate-in stagger-2" style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.82rem', marginBottom: '0.5rem', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{title}</p>
          <h2 style={{ fontSize: '1.9rem', margin: 0, color: '#fff', fontWeight: 800 }}>{value}</h2>
      ...
      {/* Corner glow */}
      <div style={{ position: 'absolute', bottom: '-20px', right: '-20px', width: '80px', height: '80px', background: 'var(--gold)', filter: 'blur(50px)', opacity: 0.06, pointerEvents: 'none' }} />
    </div>
  );
}
